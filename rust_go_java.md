...
指针就被编码为内存地址，使用usize类型的整数表示。一个地址就会指向地址空间的某个地方。

地址空间的范围是OS和CPU提供的外观界面。程序只知道有序地字节序列，不会考虑系统中实际的RAM数量 。

名词解释：

内存地址，就是指代内存中某个字节的一个数，内存地址是汇编语言提供的抽象

指针（有时扩展为原始指针），就是指向某种类型的一个内存地址。指针是高级语言提供的抽象。

引用，就是指针。如果是动态大小的类型，就是指针和额外保证的一个整数，引用是Rust提供的抽象。
